...
def classify():
	...
	pkl_file = open('naivemodel.pkl', 'rb')
...
def classifyTextWithScore2(inputText):
	pkl_file = open('naivemodel.pkl', 'rb')
	naiveClassifier = pickle.load(pkl_file)
	pkl_file.close()
	prob_dist = naiveClassifier.prob_classify(inputText)
	...
	if (confidence<0.95):
		...
		return "others"
	elif(str(prob_dist.max()== "Transfer intent"):
		if ("trans" in inputText.lower() or "pay" in inputText.lower()):
			return str(prob_dist.max())
		else:
			return "others"
	elif(str(prob_dist.max()== "Account intent"):
                if ("trans" in inputText.lower() or "pay" in inputText.lower()):
                        return str(prob_dist.max())
                else:
                        return "others" 
	elif(str(prob_dist.max()== "Transaction intent"):
                if ("trans" in inputText.lower() or "corp" in inputText.lower()):
                        return str(prob_dist.max())
                else:
                        return "others"
